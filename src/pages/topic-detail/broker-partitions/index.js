...
module.exports = template({
  ...
  computed: {
    ...
    sqls () {
      ...
      this.brokerMetrics.filter(m => m.show).forEach(m => {
        ...
        result[`Broker:${m.broker}:${m.name}`] = `SELECT mean("${key}") FROM "${m.name}" WHERE "broker" = '${m.broker}' AND "topic" = '${m.topic}' AND "partition" = '${m.partition}' AND time > now() - 1h GROUP BY time(1m)`
      })
      return result
    }
  },
  methods: {
    ...
    fetchBrokerMetrics (broker) {
      ...
      axios.get('api/brokers/' + broker + '/jmx?key=kafka.*:type=*,name=*,partition=*,topic=' + topicName).then(resp => {
